class Compressed:
    ...
    def compress(self,gene: str):
        ...
        #self.toReturn=self.bit_string
        return self.bit_string
    def decompress(self,CompressededData) -> str:
        gene: str = ""


        for i in range(0, CompressededData.bit_length() - 1, 5):  # -1 to exlude sentinel

            test = CompressededData >> i
            testBin = bin(test)
            test2 = test & 0b11

            bits: int = CompressededData >> i & 0b11111
...
if __name__ == '__main__':  # Program will start here
    ...
    CompressededData =Compressed()
    toCompress = CompressededData.compress(original)
    print("compressedData",toCompress)

    deComData = CompressededData.decompress(toCompress)
    print("deComData",deComData)
